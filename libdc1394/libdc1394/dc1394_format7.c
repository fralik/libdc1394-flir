...
static int
QueryFormat7CSROffset(dc1394camera_t *camera,
...
static int
GetCameraFormat7Register(dc1394camera_t *camera,
                         unsigned int mode, octlet_t offset, quadlet_t *value)
{
  ...
  if (camera->format7_csr[mode-MODE_FORMAT7_MIN]==0) {
    if (QueryFormat7CSROffset(camera, mode, &csr) != DC1394_SUCCESS) 
      return DC1394_FAILURE;
    else 
...
static int
SetCameraFormat7Register(dc1394camera_t *camera,
                         unsigned int mode, octlet_t offset, quadlet_t value)
{
  int retval, retry= MAX_RETRIES;
  quadlet_t csr;
