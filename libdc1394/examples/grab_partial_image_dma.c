...
**    $RCSfile$
**   $Revision: 307 $$Name$
**       $Date: 2006-10-16 02:59:41 -0400 (Mon, 16 Oct 2006) $
**   Copyright: LGPL $Author: dmoore $
** Description:
**
...
int main(int argc, char *argv[]) 
{
  ...
  err=dc1394_capture_setup_dma(camera, 8);
  ...
  */
  ...
  for( i = 0; i < grab_n_frames; ++i) {
     ...
     *-----------------------------------------------------------------------*/
    if (i == grab_n_frames - 1) {
      imagefile=fopen("Part.pgm","w");

      dc1394_get_image_size_from_video_mode(camera, DC1394_VIDEO_MODE_FORMAT7_0,
          &width, &height);
    ...
    dc1394_capture_enqueue_dma (camera, frame);
  }
